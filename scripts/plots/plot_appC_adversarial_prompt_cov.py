"""Plot change-of-verdict rates for the adversarial system-prompt experiment."""
...
OUTPUT_PATH = REPO_ROOT / "artifacts" / "figures" / "appC_adversarial_prompt_cov.pdf"
...
ADVERSARIAL_DATA_PATHS = (
    DATA_DIR / "sync_h2h_adversarial_cla_vs_gpt.parquet",
    DATA_DIR / "sync_h2h_adversarial_cla_vs_gem.parquet",
    DATA_DIR / "sync_h2h_adversarial_gpt_vs_gem.parquet",
)
MODEL_PAIRS = (
    ("Claude", "GPT"),
    ("Claude", "Gemini"),
...
def change_rate_p_value(
    ...
    """Compare original and adversarial change rates with a two-sided z-test."""
    counts = (
        change_of_minds(original[verdict_column]),
        change_of_minds(adversarial[verdict_column]),
    )
    sample_sizes = (len(original), len(adversarial))
...
def plot_model_change_rates(
    ...
    *,
    ...
    """Plot and compare one model's original and adversarial change rates."""
    estimates = (
        bootstrap_change_rate(original, verdict_column),
        bootstrap_change_rate(adversarial, verdict_column),
    )
    ...
    label = significance_label(
        change_rate_p_value(original, adversarial, verdict_column)
    )
...
def build_figure(
    ...
    adversarial: tuple[pd.DataFrame, ...],
) -> plt.Figure:
    """Build the three-panel adversarial system-prompt comparison figure."""
...
def main() -> None:
    """Load debate data, build the adversarial comparison, and save it."""
    use_latex_style()
    ...
    adversarial = tuple(load_debate_data(path) for path in ADVERSARIAL_DATA_PATHS)
    figure = build_figure(originals, adversarial)
