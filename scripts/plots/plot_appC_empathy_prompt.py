"""Plot debate outcomes for the empathy system-prompt experiment."""
...
OUTPUT_PATH = REPO_ROOT / "artifacts" / "figures" / "appC_empathy_prompt.pdf"

DATA_PATHS = (
    DATA_DIR / "sync_h2h_empathy_cla_vs_gpt.parquet",
    DATA_DIR / "sync_h2h_empathy_cla_vs_gem.parquet",
    DATA_DIR / "sync_h2h_empathy_gpt_vs_gem.parquet",
)
MODEL_PAIRS = (
...
def style_axis(axis: Axes, grid_axis: str) -> None:
    """Apply the shared empathy-figure axis styling."""
...
def plot_change_rates(axis: Axes, dataframes: tuple[pd.DataFrame, ...]) -> None:
    """Plot model-level change-of-verdict rates by debate pairing."""
    ...
    for index, (dataframe, color, model_pair) in enumerate(
        ...
        rates = [
            change_of_verdict_rate(dataframe, column) for column in VERDICT_COLUMNS
        ]
        errors = np.array(
            [
                bootstrap_errors(
...
def build_figure(dataframes: tuple[pd.DataFrame, ...]) -> plt.Figure:
    """Build the two-panel empathy system-prompt figure."""
...
def main() -> None:
    """Load empathy debate data, build Figure 16, and save it."""
    use_latex_style()
